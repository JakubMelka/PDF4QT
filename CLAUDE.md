...
| `UnitTestsAnnotationManipulator` | [tst_annotationmanipulatortest.cpp](UnitTests/tst_annotationmanipulatortest.cpp) — `PDFAnnotationManipulator`: transformation of each geometry kind (points, box, icon, appearance stream matrix), editable points, copy/move between pages, clipboard serialization; keeps [pdfannotationmanipulator.cpp](Pdf4QtLibCore/sources/pdfannotationmanipulator.cpp) at 100% line/branch/MC/DC coverage |
| `UnitTestsAnnotationSelection` | [tst_annotationselectiontest.cpp](UnitTests/tst_annotationselectiontest.cpp) — selection of annotations in `PDFWidgetAnnotationManager` (click, modifiers, rubber band), keyboard, clipboard, handles of the selection frame, rotation, point handles; needs `QT_QPA_PLATFORM=offscreen` |
