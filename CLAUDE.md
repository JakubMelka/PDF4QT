...
| `UnitTestsPDFObject` | [tst_pdfobjecttest.cpp](UnitTests/tst_pdfobjecttest.cpp) — the 16 byte `PDFObject` and `PDFInplaceOrMemoryString`: all value types and their boundaries (inplace strings of 14 characters, references with generation outside of 32 bits), type mismatch exceptions, copy/move/self-assignment/aliasing, lifetime of shared content (detected through sharing of a `QByteArray` probe), dictionaries, manipulator, and multithreaded stress tests (concurrent copies, simultaneous release of the last reference, objects destroyed in other threads, concurrent parsing). See [OPTIMIZATION.md](OPTIMIZATION.md) |
