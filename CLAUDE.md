...
On this machine there is an existing Ninja + MSVC debug tree at `build/msvc2022_debug` (Qt 6.9.0 at `E:/Programming/Qt/6.9.0/msvc2022_64`). When that tree is missing, use the Qt Creator tree `build/Desktop_Qt_6_11_2_MSVC2026_64bit_Release` (Release, Qt 6.11.2, `VCPKG_MANIFEST_INSTALL=OFF` - after a change of [vcpkg.json](vcpkg.json) run `vcpkg install --x-manifest-root=. --x-install-root=<build>/vcpkg_installed --overlay-ports=vcpkg/overlays/general` by hand). Build it by writing a `.bat` to the scratchpad (nested quotes get mangled otherwise) that calls `vcvars64.bat`, `cd /d` into the build dir and runs `ninja <target>`.
...
| `UnitTestsOCR` | [tst_ocrtest.cpp](UnitTests/tst_ocrtest.cpp) — OCR core: page ranges, confidence statistics, session editing with undo/redo, invisible text layer (round trip, pixel identical rendering, geometry over rotations/CropBox/UserUnit, idempotent apply and removal, isolation of the graphic state of foreign content, forged layer metadata), preprocessing (deskew direction, orientation), regressions of the session (undo against new results, rotated and right-to-left lines), page analysis and existing text policy, annotations/redactions, job controller (cancellation, page errors, retry), project files, text export, engine adapter contract with the built-in `test` engine, model manager (built-in set, download from a loopback http server, checksum/size/html failures) and recognition by Tesseract when it is compiled in, dictionary information of the words, hOCR / ALTO / TSV export, `PDFOCRApplyProcessor` (permissions, DocMDP, conformance, execution), compression of the scanned images in the write transaction (lossless JBIG2 generic region / CCITT G4, bitonal text scans, shared images, streaming memory, fingerprint rebinding after *Optimize Images*), perspective correction and `PDFOCRDocumentRunner` (`documentRunner`: policies, masking, review only, page errors, project reuse, reduced resolution, copy, cancellation, `processFile`). `qualityAndPerformanceBenchmark` runs only when `PDF4QT_OCR_BENCHMARK` is set (native platform plugin, `PDF4QT_OCR_BENCHMARK_PAGES`, `PDF4QT_OCR_BENCHMARK_PROFILE` = `fast` / `standard` / `best`, `PDF4QT_OCR_BENCHMARK_DEGRADED_DPI` and `PDF4QT_OCR_BENCHMARK_DEGRADED_NOISE` for a blurred and noisy scan, `PDF4QT_OCR_BENCHMARK_REPORT`; set `QTEST_FUNCTION_TIMEOUT` for long runs) |
| `UnitTestsOCRDialog` | [tst_ocrdialogtest.cpp](UnitTests/tst_ocrdialogtest.cpp) — drives `PDFOCRDocumentDialog` with the `test` engine and with Tesseract: recognize, correct, undo/redo, apply the layer, invalidation of the text layout of the widget proxy, reopening of the own layer for further corrections, a stopped repeated recognition keeps the results, dictionary review and structured export, compression with its preview, `PDFScanPreparationDialog`, perspective corners and `PDFOCRBatchDialog` (batch into a folder, skip existing, stop, open result). Both OCR tests run with `QT_QPA_PLATFORM=offscreen` and `PDF4QT_OCR_DATA_DIRECTORY=<build>/usr/bin/ocr` (set by ctest; the language models live there, not in the source tree); missing Tesseract or models skip the Tesseract tests via `PDF4QT_OCR_SKIP`, which is a failure when the tree is configured with `PDF4QT_OCR_REQUIRED=ON` (release gate) |
| `UnitTestsScanPreparation` | [tst_scanpreparationtest.cpp](UnitTests/tst_scanpreparationtest.cpp) — `PDFScanPreparation`: deskew sign and center over `/Rotate` / CropBox / `UserUnit`, unbalanced content without `q` / `Q`, split pages sharing the content, reading order and `/PageLabels`, split + deskew + crop, tagged pages are not split, decision for pages with an own OCR layer, image and page analysis (skew, content box, gutter), plan validation |
| `PdfToolOcrSmoke` | [pdftoolocrsmoke.cmake](UnitTests/pdftoolocrsmoke.cmake) — CMake script test of `PdfTool ocr` / `ocr-models` with Tesseract and the built-in models: a text PDF is rasterized by `PdfTool bitonal`, recognized, the layer, all exports, the project, the own layer policy, compression, a batch with a broken file and the download consent are checked. Skipped (`PDF4QT_OCR_SKIP`) without the engine or the models, unless `PDF4QT_OCR_REQUIRED=ON` |
...
Specified by [OCR.md](OCR.md) (Czech), implementation report in [OCR_REPORT.md](OCR_REPORT.md). The engine independent part lives in Pdf4QtLibCore (`pdfocr*.h`): data model in canonical unrotated PDF user space (`PDFOCRPageResult`, quads ordered bottom-left, bottom-right, top-right, top-left of the reading direction), `PDFOCREngine` / `PDFOCREngineFactory` / `PDFOCREngineRegistry` (engines return raw pixel-space output, `PDFOCRPagePreparer` maps it back through `pageToRaster * rasterToEngine`), `PDFOCRPagePreparer` (analysis: `textRectangles` of the existing digital / foreign invisible text, a scan with any such text is `Mixed` and needs a decision; rasterization honours `/UserUnit` and the engine dimension limit; the page fingerprint `PDF4QT-OCR-PAGE-FINGERPRINT-2` covers the full raw streams of the resources, the annotations and the default optional content configuration, excluding only the verified own layer streams; overlapping inclusive regions with the same configuration are united), `PDFOCRJobController` (worker pool, one engine instance per worker, memory budget, cancellation), `PDFOCRSession` (results, corrections, undo/redo), `PDFOCRTextLayerWriter` (invisible text with an embedded glyphless font shared by the pages, the text origin lies on the line baseline and a negative `Ts` shifts the glyph boxes back onto the word geometry; foreign content is enclosed between `q` / `Q` isolation streams - shared when the foreign content is balanced, page specific with extra `ET` / `EMC` / `Q` closers otherwise, `computeContentBalance` / `validatePageContent` check the nesting by the lexer; own layer identified by the page `/PieceInfo /PDF4QT_OCR` - its references are untrusted input, the content and data streams are bound by `ContentHash` / `DataHash` (SHA-256 of the decoded streams) and the content must pass `isInvisibleTextStream`, a stream changed by another tool is foreign content: it is part of the fingerprint, never removed and `readLayer` returns nothing; `apply` refuses `reviewOnly` results and words colliding with `analysis.textRectangles`, and recomputes the exclusion flags from the current regions; `PDFPage::getContents()` is already dereferenced, use `getPageContentReferences`), `PDFOCRModelManager` (catalog with pinned checksums in [ocr/catalog/](ocr/catalog/), three profiles `fast` / `standard` / `best` = repositories tessdata_fast / tessdata / tessdata_best, iterate them with `PDFOCRConfiguration::getProfiles()`, built-in models in `ocr/tesseract/<profile>/` (only [fast](ocr/tesseract/fast/) is distributed, the larger profiles are downloaded by the user) - the model files are committed as one xz archive per model (`tessdata/<language>.tar.xz`, 13 MB instead of 35 MB) and CMake extracts them into `<build>/<PDF4QT_INSTALL_OCR_DIR>`, which is what is installed and what the OCR tests read; the catalog, the manifests, the licenses and the archives are generated by [ocr/tools/generate_catalog.py](ocr/tools/generate_catalog.py), `--fetch-builtin` restores a missing or damaged archive without the 3 GB cache, a run without it regenerates the catalog and caches all models (3 GB), `--<profile>-commit` moves a pinned commit, `--builtin-<profile>` changes a built-in set (then update [WixInstaller/Product.wxs.in](WixInstaller/Product.wxs.in) by hand); https downloads, imports, atomic runtime sets) and `PDFOCRProject`. The Tesseract adapter is the separate shared library [Pdf4QtOcrTesseract/](Pdf4QtOcrTesseract/) (option `PDF4QT_ENABLE_OCR`, define `PDF4QT_OCR_TESSERACT`), nothing else may include Tesseract or Leptonica headers. The dialogs (`PDFOCRDocumentDialog`, `PDFOCRLanguagesDialog`, `PDFOCRPageView`) are in Pdf4QtLibGui; the `.ui` files of the two dialogs are large, keep object names stable because the dialog test looks widgets up by name.

Extensions of [OCR_PLAN.md](OCR_PLAN.md) (chapter 10 of the report): `PDFOCRApplyProcessor` (`pdfocrapplyprocessor.h`) is the UI independent write transaction used by the dialog and the command line (verify fingerprints → compress images → write layer → strip conformance of a copy → validate → write copy); `PDFOCRImageCompressor` / `PDFOCRCompressionSettings` (`pdfocrcompression.h`, JBIG2 only as a generic region, never a symbol dictionary; lossy modes need the preview in the dialog and are not offered by the batch dialog); `PDFOCRTextLayerWriter::rebindFingerprints` rebinds an own layer when only the image encoding changed (also called by `PDFImageOptimizer`); `PDFOCRStructuredExporter` (`pdfocrexport.h`, coordinates of the visible page at the export DPI); `PDFOCRWord::inDictionary` + `PDFOCRReviewCriteria` (words outside the dictionary of the model need a review, default on, no extra dictionary library); `PDFOCRPreprocessing::perspective` (projective working raster, words converted back to parallelograms by `toParallelogram`); `PDFScanPreparation` (`pdfscanpreparation.h`, crop / split / deskew of the visible pages before OCR, wrapper streams, cloned page dictionaries sharing the content, the caller decides about pages with an own layer) with `PDFScanPreparationDialog`; `PDFOCRDocumentRunner` (`pdfocrdocumentrunner.h`, synchronous headless recognition with its own event loop - call it from a thread with an event dispatcher - and `processFile` for a single file) used by `PdfTool ocr` ([pdftoolocr.cpp](PdfTool/pdftoolocr.cpp), also `ocr-models`, never downloads without `--accept-download`) and `PDFOCRBatchDialog` (worker `QThread`). The OCR data root is shared by all PDF4QT applications: `PDFOCRModelManager::getApplicationDataRoot` maps the application name of organization `MelkaJ` to `PDF4QT Editor`, so PdfTool sees the models downloaded in the editor. PdfTool links `Pdf4QtOcrTesseract` only with `PDF4QT_ENABLE_OCR` and registers the engine in `main`.
